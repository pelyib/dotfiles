...
M.get_git_branch = function()
	...
	if
		cache.git_branch.value
		and cache.git_branch.cwd == current_cwd
		and (current_time - cache.git_branch.last_update) < cache.git_branch.ttl
	then
...
M.get_file_status = function()
	...
	-- Check if cache is valid
	if
		cache.file_status.value
		and cache.file_status.file_path == current_file
		and (current_time - cache.file_status.last_update) < cache.file_status.ttl
	then
...
M.get_code_context = function()
	...
	if
		...
		and cache.code_context.last_line == line
		and cache.code_context.last_col == col
		and cache.code_context.value
	then
...
M.format_special_buffer = function()
	...
	return string.format("%s%s[%s]", branch, branch ~= "" and M.config.icons.separator or "", root)
end
